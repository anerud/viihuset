...
class BoardChatAdminReplyView implements iView{
	...
	public function render(){
				...
				$values = array();
				array_push($values, new TitleSubtitleListItem($this->thread->message,"SKAPAD AV: ".$this->thread->poster." - ".$this->thread->posted, null));	
				...
				foreach ($this->thread->replies as $reply) {
					...
					array_push($values, new TitleSubtitleListItem($reply->message,"SVAR AV: ".$reply->poster." - ".$reply->posted, $headerStyles[0]));
					array_push($values, new JSDeleteListItem("/styrelsechat/".$this->thread->id."/".$reply->id, $headerStyles[1]));
