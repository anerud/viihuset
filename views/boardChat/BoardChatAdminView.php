...
class BoardChatAdminView implements iView{
	...
	public function render(){
				...
				$headers = array("anslag","antal inlägg","radera");
				...
				foreach ($this->threads as $thread) {
					...
					array_push($values, new LinkListItem("/styrelsechat/".$thread->id ,new TitleSubtitleListItem($thread->title,"AV: ".$thread->poster." - ".$date, $headerStyles[0]),false));
					array_push($values, new TextListItem("(".$thread->repliesCount.")", $headerStyles[1]));
					array_push($values, new JSDeleteListItem("/styrelsechat/".$thread->id, $headerStyles[2]));
